...
| 8 | Report template | Analyst can upload a custom Word template (e.g., company-branded). Templates can be personal (private) or shared at department level. System fills in content via Jinja2 placeholders. |
...
| # | Feature | Decision |
|---|---------|---------|
| 1 | Institutional data management | Option C: Core datasets (performance reports, customer records, transaction logs, usage data, survey/complaint data) managed by IT/data team via admin panel. Analysts can supplement with personal file uploads. Upload capability is ON by default with an admin toggle to disable. |
...
**Future scope:**
- LLM provider switching (Gemini → other OpenAI-compatible providers)
- Enterprise data catalog connector configuration
...
| Q6 | Data versioning | ✅ **On by default for all datasets.** Every analysis result is automatically linked to a point-in-time snapshot of the data that produced it. No exceptions. This makes every finding reproducible — if a result is ever questioned, the exact data snapshot that produced it must be retrievable. |
...
| 3 | Mid-analysis steering | Analyst can inject a plain-English hint while the task is running (e.g., "Focus on the retail channel only, ignore wholesale"). Hint is appended to the Planner's context for all subsequent rounds. Multiple hints accumulate — they are not overwritten. All hints visible in the real-time progress panel with timestamps. V1: hints apply from next round onward only (no backtracking). V2: smart backtracking when hint contradicts current plan direction. |
...
| 5 | DS-STAR+ output feel | Formal document — structured sections matching business review / thematic review report formats used today. Department-specific templates govern section structure. |
...
| 3 | Department report adaptation | Department templates control branding, header/footer, document classification markings, and tone. They do not mandate a fixed section structure. "Recommendations" becomes "Action Items" in some department templates. |
...
| 6 | Report audience | Primarily internal (analyst, their manager, department head). Reports going to external partners get a "formal tone" modifier in the Writer's prompt — but same structure. |

---

## Topic 6: Department-Specific Features

🔮 **Deferred to V2.** V1 treats all analysts identically regardless of department. No department-specific UI, starter templates, data schemas, domain knowledge injection, or output templates in V1. Every analyst sees the same experience. Department-specific customisation is a future scope item to be designed when the core system is proven.

---

## Topic 7: PII Handling

🔮 **Deferred to V2.** V1 data sources are analyst-uploaded files (Excel, CSV, PDF) rather than live production databases. PII masking is not implemented in V1 — analysts are responsible for the sensitivity of data they upload. When live production databases are connected in a future version, PII masking rules will be designed at that point based on the actual data schemas and the organization's data governance policy.
...
| 3 | Monitoring & alerts | Purely passive. The audit log is a compliance record, not a monitoring tool. No alerting, anomaly detection, or active review in V1. Available on demand for internal review. |
